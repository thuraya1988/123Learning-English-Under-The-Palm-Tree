...
// requires the shared teacher PIN, checked against the CLASS_STARS_PIN
// secret set on this function — never hardcoded here.
...
function requirePin(body: any): boolean {
  const pin = clean(body.pin, 20);
  const expected = Deno.env.get("CLASS_STARS_PIN") || "";
...
Deno.serve(async (req) => {
  ...
  if (!requirePin(body)) return json({ ok: false, error: "forbidden" }, 403);
