...
Deno.serve(async(req)=>{
  ...
  if(action==="save_duty_roles"){
    ...
    const teachers:string[]=d.teachers||[],allowed=elevated(e)||teachers.some((n:string)=>norm(n)===norm(e.short_name)||norm(n)===norm(e.full_name));if(!allowed)return json({ok:false,error:"not_on_duty"},403);
  ...
  if(action==="report_class"){
    ...
    if(status==="no_teacher"&&period&&period>=1&&period<=7){
      ...
      if(coverage?.replacement_employee_id){const {data:replacement}=await db.from("multaqa_employees").select("short_name,full_name").eq("employee_id",coverage.replacement_employee_id).maybeSingle();targetName=replacement?.short_name||replacement?.full_name||targetName}
      if(targetName){notifiedTeacher=targetName;sent=await pushToNames(db,[targetName],"🚨 صف بدون معلمة",`الصف ${classLabel} • الحصة ${period}. يرجى التوجه إلى الصف الآن.`,"/school/","class_alert")}
    }
