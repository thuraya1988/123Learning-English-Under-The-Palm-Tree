...
export function addShipEdges(mesh, thr, mat) {
  mesh.add(new THREE.LineSegments(new THREE.EdgesGeometry(mesh.geometry, thr || 22), mat || shipEdgeMat));
}

/* neon edge-glow liveries — same real airframe, different panel-line
   colors the user can pick from (additive blending so the lines read
   as glowing against the dark background instead of flat paint). */
export const NEON_LIVERIES = {
...
function makeNeonEdgeMat(liveryKey) {
  const p = NEON_LIVERIES[liveryKey] || NEON_LIVERIES.classic;
  return new THREE.LineBasicMaterial({
    color: p.color, transparent: true, opacity: p.opacity,
    blending: p.glow ? THREE.AdditiveBlending : THREE.NormalBlending, depthWrite: false,
  });
}
...
export function buildShip(liveryKey) {
  const edgeMat = makeNeonEdgeMat(liveryKey);
