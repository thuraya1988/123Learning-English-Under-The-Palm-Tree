...
const ROSTER_ALIASES: Record<string, string> = {"فداء بنت احمد بن سعيد عبدالله الزكوية":"فداء بنت احمد بن سعيد عبدالله الأزكوية"};
...
Deno.serve(async (req) => {
  ...
  if (action === "import") {
    ...
    const items = source.flatMap((item: any) => {
      const cls = clean(item?.cls, 20), rawName = clean(item?.name, 200), name = ROSTER_ALIASES[rawName] || rawName;
  ...
  if (action === "save") {
    const cls = clean(body.cls, 20), rawName = clean(body.name, 200), name = ROSTER_ALIASES[rawName] || rawName;
