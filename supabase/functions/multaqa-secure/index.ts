...
Deno.serve(async(req)=>{
  ...
  if(action==="analytics"){
    ...
    const [{data:students},{data:teachers},{data:permissions},{data:duties},{data:observations}]=await Promise.all([
  ...
  if(action==="student_case"){
    const sid=clean(body.student_school_id,30);const {data:s}=await db.from("multaqa_students").select("school_id,name,class_name,guardian_phone,area").eq("school_id",sid).maybeSingle();if(!s)return json({ok:false,error:"not_found"},404);const [{data:c},{data:n},{data:t}]=await Promise.all([db.from("multaqa_student_cases").select("*").eq("student_school_id",sid).maybeSingle(),db.from("multaqa_student_case_notes").select("id,employee_id,subject_label,note,visibility,created_at").eq("student_school_id",sid).order("created_at",{ascending:false}),db.from("multaqa_student_talents").select("*").eq("student_school_id",sid)]);const safeCase=caseTeam(e)?c:(c?{academic_level:c.academic_level,indicators:c.indicators,follow_up_issue:c.confidential?null:c.follow_up_issue}:null);const notes=caseTeam(e)?(n||[]):(n||[]).filter((x:any)=>x.visibility==="all_teachers"||x.employee_id===e.employee_id);return json({ok:true,student:{school_id:s.school_id,name:s.name,class_name:s.class_name,area:caseTeam(e)?s.area:null,guardian_phone:caseTeam(e)?s.guardian_phone:null},case:safeCase,notes,talents:t||[]});
  }
  if(action==="save_student_case"){
    if(!caseTeam(e))return json({ok:false,error:"forbidden"},403);const sid=clean(body.student_school_id,30);const row={student_school_id:sid,residence:clean(body.residence,200)||null,contact_phone:clean(body.contact_phone,30)||null,social_status:clean(body.social_status,500)||null,health_status:clean(body.health_status,500)||null,follow_up_issue:clean(body.follow_up_issue,1200)||null,academic_level:clean(body.academic_level,20)||null,indicators:Array.isArray(body.indicators)?body.indicators.map((x:any)=>clean(x,100)).filter(Boolean).slice(0,20):[],confidential:body.confidential!==false,assigned_to_employee_id:clean(body.assigned_to_employee_id,30)||null,updated_by_employee_id:e.employee_id,updated_at:new Date().toISOString()};const {error}=await db.from("multaqa_student_cases").upsert(row);if(error)return json({ok:false,error:"save_failed"},500);return json({ok:true});
  }
